...
abstract class Olap4jNodeConverter {
        ...
        List<List<ParseTreeNode>> contextUnions)
    {
        ...
        QueryHierarchy qDim = axis.getDimensions().get(dim);
    ...
    private static AxisNode toOlap4j(QueryAxis axis) {
        ...
        if (numDimensions == 0) {
            ...
            QueryHierarchy dimension = axis.getDimensions().get(0);
    ...
    private static List<ParseTreeNode> toOlap4j(QueryHierarchy dimension) {
        ...
        if (dimension.getHierarchizeMode() != null) {
            ...
            if (dimension.getHierarchizeMode().equals(
                    QueryHierarchy.HierarchizeMode.PRE))
            {
                ...
                hierarchyNode = new CallNode(
                    ...
                    QueryHierarchy.HierarchizeMode.POST))
            {
                hierarchyNode = new CallNode(
                    null,
                    "Hierarchize",
